...
surface.CreateFont("BROOP_18", {font = "CloseCaption_Normal", size = 18, weight = 400, blursize = 0, antialias = true, shadow = false})
surface.CreateFont("BROOP_18Blur", {font = "CloseCaption_Normal", size = 18, weight = 400, blursize = 0, antialias = true, shadow = false})

surface.CreateFont("BROOP_16", {font = "CloseCaption_Normal", size = 16, weight = 400, blursize = 0, antialias = true, shadow = false})
surface.CreateFont("BROOP_16Blur", {font = "CloseCaption_Normal", size = 16, weight = 400, blursize = 0, antialias = true, shadow = false})
...
local ply, wep, vm, attachment, FT, CT, vel, runspeed, eyeang, activity, str, ShakeSTR, WeaponAnglePitch, WeaponAngleRoll, func, x, y, aim, ang, mag, clr, EP, EA, r, calc, arm, re, att, old, x, y, f, m, c, s1, s2, add, hp, c2
...
local function CSTM_InitPostEntity()
	...
	timer.Simple(10, function()
		chat.AddText(Color(141, 201, 0, 255), "-= Customizable Weaponry help =-")
		chat.AddText(Color(255, 255, 255, 255), "If you're having problems figuring out the controls, type ", Color(141, 201, 0, 255), "cstm_controls", Color(255, 255, 255, 255), " into your console.")
		chat.AddText(Color(255, 255, 255, 255), "If you have a question that needs answering, type ", Color(141, 201, 0, 255), "cstm_faq", Color(255, 255, 255, 255), " into your console.")
	end)
end
...
local function CSTM_HUDShouldDraw(n)
	if n == "CHudDeathNotice" then
		return false
	end
	...
	if IsValid(ply) and ply:Alive() then
		...
		if IsValid(wep) and wep.IsCSTMWeapon then
			if GetConVarNumber("cstm_hud_ammo") > 0 then
...
hook.Add("HUDShouldDraw", "CSTM_HUDShouldDraw", CSTM_HUDShouldDraw)
	...
	if IsValid(wep) then
		if wep.IsCSTMWeapon then
			...
			if wep.VElements then
				c = false
				s1, s2 = wep.VElements["acog"], wep.VElements["ballistic"]
				
				if s1 and s1.color.a == 255 then
					FT = FrameTime()
					f = 4
					c = true
				elseif s2 and s2.color.a == 255 then
					FT = FrameTime()
					wep.CurFOV = app(wep.CurFOV, wep.TargetFOV, FT * 10)
					f = wep.CurFOV
					c = true
				end
				...
				if c then
					if wep:GetDTInt(3) != 1 or not wep.DrawTable then
						BlackAmt = app(BlackAmt, 1, FT * 6)
					else
						BlackAmt = app(BlackAmt, 0, FT * 6)
					end
...
local lastanim = ""
local lastwep = ""
local att, x, y, atts, cl, dt3, ammo, al
					...
					if atts > 0 then
						for k, v in ipairs(wep.Attachments) do
							...
							if not att or att.x > x or att.y > y or att.x < 0 or att.y < 0 then
								...
								PA:SetPos(att.x + 200, att.y + PA.Off - 50)
							end
...
local function TWeps_ViewShake(ply, pos, angles, fov)
	...
	if ply.attachment then
			if GetConVarNumber("cstm_viewbob_reload") > 0 and (activity == "ACT_VM_RELOAD" or activity == "ACT_VM_RELOAD_SILENCED") or GetConVarNumber("cstm_viewbob_silencer") > 0 and (activity == "ACT_VM_ATTACH_SILENCER" or activity == "ACT_VM_DETACH_SILENCER") or GetConVarNumber("cstm_viewbob_draw") > 0 and (activity == "ACT_VM_DRAW" or activity == "ACT_VM_DRAW_SILENCED") or wep.ViewMoveWhenFiring and activity == "ACT_VM_PRIMARYATTACK" then
			cyc = vm:GetCycle()
	...
	if animplayed then
		WeaponAngle = LerpAngle(FT * 5, WeaponAngle, ply.attachment.Ang * 0.15)
	else
	...
	view.angles.y = view.angles.y - (WeaponAngle.p + WeaponAngle.y) * 0.05 * CurWeaponAngleMod * clamp(GetConVarNumber("cstm_yawmod"), 0.2, 2)
	view.angles.r = view.angles.r + WeaponAngle.p * (wep.PitchMod or 1) * CurWeaponAngleMod * clamp(GetConVarNumber("cstm_rollmod"), 0.2, 2)
	view.origin = pos
...
local function PickUpAttachment(um)
	...
	chat.AddText(Color(255, 255, 255), "Got new weapon attachment: ", Color(50, 150, 255), AttToText[s], Color(255, 255, 255), ".")
end
...
local function ReceiveWeaponRecoil(um)
	s = um:ReadFloat()
	wep = LocalPlayer():GetActiveWeapon()
	
	wep.Primary.Recoil = s
end

usermessage.Hook("CSTM_RECOIL", ReceiveWeaponRecoil)
...
local function PickUpAmmo(um)
	...
	chat.AddText(Color(255, 255, 255), "Got new ammo type: ", Color(50, 150, 255), AmmoTypes[s].name, Color(255, 255, 255), ".")
end
