					...
					if self.AmmoCharge > 0 then
						...
						if am != -1 then
							...
							if wep.Primary and wep.Primary.ClipSize then
								if wep.Primary.ClipSize * 6 > amc then
									self.AmmoCharge = self.AmmoCharge - 1
									v:EmitSound("items/ammo_pickup.wav", 60, 100)
									
									ammo = math.Clamp(amc + (wep.Primary.ClipSize > 50 and wep.Primary.ClipSize / 2 or wep.Primary.ClipSize), 0, wep.Primary.ClipSize * 6)
									v:SetAmmo(ammo, am)
								end
							end
						end
						...
						if v:GetAmmoCount("Grenade") < 6 then
